...
public class BasicPyramidSolitaire implements PyramidSolitaireModel<Card> {
  ...
  protected List<List<Card>> pyramidOfCards;
  protected List<Card> stock;
  protected Card[] drawPile;
  protected boolean isGameStarted;
  protected int deckCount;
  protected boolean isMultiPyramid;
  ...
  public List getDeck() {
    ...
    for (int count = 0; count < Card.DECK_COUNT; count++) {
      deckList.add(new Card(Card.FACEVALUE[count % groupSize], Card.SYMBOL[count / groupSize]));
    }
    if (isMultiPyramid) {
      for (int count = 0; count < Card.DECK_COUNT; count++) {
        deckList.add(new Card(Card.FACEVALUE[count % groupSize], Card.SYMBOL[count / groupSize]));
  ...
  public void remove(int row, int card) throws IllegalArgumentException, IllegalStateException {
    ...
    if (row > lastRow || row < 0 || card < 0 || card >= pyramidOfCards.get(row).size()) {
  ...
  protected boolean isVisible(int row, int card) {
    int lastRow = pyramidOfCards.size() - 1;
    if (row == lastRow) {
      return true;
    }
  ...
  protected int getIntValue(String k) {
    switch (k) {
      case "A":
        return 1;
      case "J":
  ...
  public String toString() {
    ...
    for (int i = 0; i < size; i++) {
      int rowSize = pyramidOfCards.get(i).size();
   ...
   */
  protected boolean isDuplicated(List<Card> deck) {
